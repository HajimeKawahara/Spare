...
def dF_dx(data, A, x_d):
    ...
    return cp.einsum("ijkcl,il->jkc", A, data_dif)

def F_like(data, A,  x_d):
    data_dif = data -  cp.einsum("ijkcl,jkc->il", A, x_d)
    return (cp.einsum("il,il->",data_dif, data_dif)/2) 
...
def mfista_func(np_I_init, np_d, np_A_ten, lambda_gl,L_init= 1e4, eta=1.1, maxiter= 10000, max_iter2=100, miniter = 100, TD = 30, eps = 1e-5, print_func = False):
