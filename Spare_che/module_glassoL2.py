...
def dF_dx(data, A, x_d, lambda_tik):
    ...
    return np.einsum("ijkcl,il->jkc", A, data_dif) + 2.0*lambda_tik*x_d

def F_likeL2(data, A,  x_d, lambda_tik):
    data_dif = data -  np.einsum("ijkcl,jkc->il", A, x_d)
    L2=lambda_tik*np.einsum("jkc,jkc->", x_d, x_d)
    return (np.einsum("il,il->",data_dif, data_dif)/2) + L2
...
def mfista_func(I_init, d, A_ten, lambda_gl,lambda_tik,L_init= 1e4, eta=1.1, maxiter= 10000, max_iter2=100, miniter = 100, TD = 30, eps = 1e-5, print_func = False):
